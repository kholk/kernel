...
#define PLL_OPMODE_VAL(p)	((p)->offset + (p)->regs[PLL_OFF_OPMODE_VAL])
# define PLL_STANDBY		0x0
# define PLL_RUN		0x1

#define PLL_FRAC_VAL(p)		((p)->offset + (p)->regs[PLL_OFF_FRAC_VAL])
...
const u8 clk_alpha_pll_regs[][PLL_OFF_MAX_REGS] = {
	...
	[CLK_ALPHA_PLL_TYPE_FABIA] =  {
		...
		[PLL_OFF_TEST_CTL_U] = 0x20,
		[PLL_OFF_STATUS] = 0x24,
		[PLL_OFF_OPMODE_VAL] = 0x2c,
		[PLL_OFF_FRAC_VAL] = 0x38,
	},
	[CLK_ALPHA_PLL_TYPE_TRION] =  {
		...
		[PLL_OFF_STATUS] = 0x24,
		[PLL_OFF_OPMODE_VAL] = 0x38,
		[PLL_OFF_FRAC_VAL] = 0x40,
...
static int alpha_fabia_pll_latch_input(struct clk_alpha_pll *pll,
					struct regmap *regmap)
{
	...
	udelay(1);
	regmap_read(regmap, PLL_MODE(pll), &regval);
	...
	ret = regmap_update_bits(regmap, PLL_MODE(pll), PLL_UPDATE, 0);
	if (ret)
		return ret;
				...
				const struct alpha_pll_config *config)
{
	...
	if (config->frac)
		regmap_write(regmap, PLL_FRAC_VAL(pll), config->frac);
	 ...
	 */
	if (pll->inited)
		alpha_fabia_pll_latch_input(pll, regmap);
...
static long alpha_fabia_pll_round_rate(struct clk_hw *hw, unsigned long rate,
				     unsigned long *prate)
{
	struct clk_alpha_pll *pll = to_clk_alpha_pll(hw);
	u32 l;
	...
	return alpha_pll_round_rate(rate, *prate, &l, &a, pll_alpha_width(pll));
}

static int alpha_fabia_pll_enable(struct clk_hw *hw)
{
	int ret;
	struct clk_alpha_pll *pll = to_clk_alpha_pll(hw);
	u32 val;
	...
	regmap_write(pll->clkr.regmap, PLL_OPMODE_VAL(pll), PLL_STANDBY);
	...
	regmap_write(pll->clkr.regmap, PLL_OPMODE_VAL(pll), PLL_RUN);
...
static void alpha_fabia_pll_disable(struct clk_hw *hw)
{
	int ret;
	struct clk_alpha_pll *pll = to_clk_alpha_pll(hw);
	u32 val, mask;
...
static unsigned long
alpha_fabia_pll_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	struct clk_alpha_pll *pll = to_clk_alpha_pll(hw);
	...
	regmap_read(pll->clkr.regmap, PLL_L_VAL(pll), &l);
	regmap_read(pll->clkr.regmap, PLL_FRAC_VAL(pll), &frac);

	return alpha_pll_calc_rate(prate, l, frac, pll_alpha_width(pll));
}

static int alpha_fabia_pll_set_rate(struct clk_hw *hw, unsigned long rate,
				  unsigned long prate)
{
	struct clk_alpha_pll *pll = to_clk_alpha_pll(hw);
	unsigned long rrate;
	...
	regmap_write(pll->clkr.regmap, PLL_L_VAL(pll), l);
	regmap_write(pll->clkr.regmap, PLL_FRAC_VAL(pll), a);

	ret = alpha_fabia_pll_latch_input(pll, pll->clkr.regmap);
	return ret;
}
					...
					struct regmap *regmap)
{
	...
	ret |= regmap_read(regmap, PLL_OPMODE_VAL(pll), &opmode_val);
	if (ret)
		return 0;
				...
				const struct alpha_pll_config *config)
{
	...
	regmap_write(regmap, PLL_OPMODE_VAL(pll), PLL_STANDBY);
...
const struct clk_ops clk_alpha_pll_fabia_ops = {
	...
	.is_enabled = clk_alpha_pll_is_enabled,
	.recalc_rate = alpha_fabia_pll_recalc_rate,
	.round_rate = alpha_fabia_pll_round_rate,
	.set_rate = alpha_fabia_pll_set_rate,
};
EXPORT_SYMBOL_GPL(clk_alpha_pll_fabia_ops);

const struct clk_ops clk_alpha_pll_fabia_fixed_ops = {
	.enable = alpha_fabia_pll_enable,
	.disable = alpha_fabia_pll_disable,
	.is_enabled = clk_alpha_pll_is_enabled,
	.recalc_rate = alpha_fabia_pll_recalc_rate,
...
EXPORT_SYMBOL_GPL(clk_alpha_pll_fabia_fixed_ops);
...
static int clk_alpha_v2_pll_postdiv_set_rate(struct clk_hw *hw,
...
const struct clk_ops clk_alpha_v2_pll_postdiv_ops = {
	.recalc_rate = clk_alpha_pll_postdiv_recalc_rate,
	.round_rate = clk_alpha_pll_postdiv_round_rate,
	.set_rate = clk_alpha_v2_pll_postdiv_set_rate,
};
EXPORT_SYMBOL_GPL(clk_alpha_v2_pll_postdiv_ops);
