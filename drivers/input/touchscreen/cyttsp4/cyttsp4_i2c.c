	...
	const struct i2c_device_id *i2c_id)
{
	...
	if (IS_ERR(vdd)) {
		...
		if(retval) {
			...
			retval = regulator_set_mode(vdd, 15000);
			if (retval < 0) {
				printk("%s: regulator_set_mode vdd falied!\n", __func__);
			} else {
	...
	if (IS_ERR(vreg_l27)) {
		...
		if(retval) {
			...
			retval = regulator_set_mode(vreg_l27, 15000);
			if (retval < 0) {
				printk("%s: regulator_set_mode vreg_l27 falied!\n", __func__);
			} else {
