...
#ifdef CONFIG_QPNP_SMBFG_NEWGEN_EXTENSION
	if (chg->micro_usb_mode && vbus_rising)
		smblib_micro_usb_plugin(chg, vbus_rising);
#else
	if (chg->micro_usb_mode)
		smblib_micro_usb_plugin(chg, vbus_rising);
...
#ifdef CONFIG_QPNP_SMBFG_NEWGEN_EXTENSION
static bool usb_ever_removed = false;
#endif
...
static void smblib_handle_typec_removal(struct smb_charger *chg)
{
	...
	vote(chg->usb_icl_votable, SOMC_APSD_VOTER, false, 0);
	...
	vote(chg->usb_icl_votable, TYPEC_VOTER, false, 0);

	usb_ever_removed = true;
#endif /* CONFIG_QPNP_SMBFG_NEWGEN_EXTENSION */
...
static bool smblib_is_legacy_cable(struct smb_charger *chg)
{
	...
	return (stat5 & TYPEC_LEGACY_CABLE_STATUS_BIT);
}
#endif  /* CONFIG_QPNP_SMBFG_NEWGEN_EXTENSION */
